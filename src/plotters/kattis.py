...
IFILE = streams.KATTIS
TARGET = ("2020-12-01", 800.0) # manual entry for now

def generate(slope=False):
    if not IFILE.endswith(".xlsx"):
        raise FileImportError("Only .xlsx files supported")

    df = pd.read_excel(IFILE, sheet_name="Kattis")
    diffs = df["Difficulty"].to_list()
    dates = df["Date"].to_list()

    # ignore nan data
    data = filter(lambda p: type(p[0]) is str, zip(dates, diffs))
    ...
    del diffs # to prevent confusion
    ...
    def plot(fname, start, end, ylim=None):
        ...
        plt.plot([x3, deadline], [y3, target], "--", color=c1, label="Required progress = {} points/day".format(round(m_req,1)))
        plt.plot(dates, ratings, color=c2)
        plt.plot([x3, x2], [y3, y2], "r--", color=c2, label="Current progress = {} points/day".format(round(m, 1)))

        # Labels
        plt.ylabel("Kattis points")
        plt.xticks([
        ...
        plt.xlim([start, end])
        plt.ylim(ylim if ylim else [ratings[-1]-100, ratings[-1]+100])
    ...
    plot("kattis_progress", dt.datetime(2020, 1, 1), deadline, [0, 800])
    plot("kattis_progress_zoom", now-window, now+window)
